// Builds HunterPetLog/Data/Skins.lua from Petopia Classic (https://www.wow-petopia.com/classic/).
...
const OUT = path.join(ROOT, "HunterPetLog", "Data", "Skins.lua");
...
async function get(url, cacheName) {
  ...
  for (let attempt = 1; ; attempt++) {
    ...
    try {
      const res = await fetch(url, { headers: { "User-Agent": "Mozilla/5.0 (HunterPetLog data build)" } });
...
async function main() {
  ...
  const lines = [];
  lines.push("-- Hunter Pet Log skin database.");
  ...
  lines.push("");
  lines.push("HunterPetLog_Skins = {}");
  lines.push("");
  lines.push("local function add(family, model, id, name, npcs)");
  lines.push("  table.insert(HunterPetLog_Skins, { family = family, model = model, id = id, name = name, npcs = npcs })");
  lines.push("end");
  lines.push("");
